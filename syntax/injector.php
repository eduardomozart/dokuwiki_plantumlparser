...
class syntax_plugin_plantumlparser_injector extends DokuWiki_Syntax_Plugin {
    ...
    public function render($mode, Doku_Renderer $renderer, $data) {
        ...
        case 'xhtml':
            ...
            if(strlen($data['svg']) > 0 && !(preg_match("/(?i)(@startlatex|@startmath|<math|<latex|ditaa)/", $data['markup']))) {
    ...
    protected function _render_odt(Doku_Renderer $renderer, $state, $txtdata) {
        // if($state === DOKU_LEXER_UNMATCHED) {
			if(strlen($txtdata['svg']) > 0 && !(preg_match("/(?i)(@startlatex|@startmath|<math|<latex|ditaa)/", $txtdata['markup']))) {
